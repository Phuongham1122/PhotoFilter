...
public class PhotoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        PhotoViewHoder viewHoder = (PhotoViewHoder) holder;

        Glide.with(viewHoder.photoView.getContext())
                .load(Uri.parse(photo.getUri()))
                .placeholder(R.drawable.ic_launcher_background)
                .into(viewHoder.photoView);

        viewHoder.photoView.setOnClickListener(v -> {
...
//                            activity.ogBmp = new BitmapDrawable(activity.getResources(),resource);
        ...
        if(selectedPos == holder.getAdapterPosition()) {
            viewHoder.photoView.setAlpha(0.25f);
        }else {
            viewHoder.photoView.setAlpha(1.0f);
        }
    }
